...
- **Enum-linearity propagation** — closed (2026-05-27). Enum types whose variants carry linear payloads now resolve as `mode: "linear"`. Named bindings of such enums are enforced at all scope exits: function body, `if`/`else` branches, `match` arms, and `loop` bodies. Match scrutinees that are linear enum bindings are consumed by the match. Branch-merge (`mergeScopes`) correctly sees linear enums and enforces all-branch consumption. The move/lend classification pipeline in `buildTypeEnv` covers linear enums: explicit `move`/`lend` annotations are respected, body-based inference runs, and externs with unannotated linear enum params emit `BuildError`. See `tests/should_fail/enum_value_leak.fit` (rejected) and `tests/should_pass/enum_value_matched.fit` (accepted). The `"linear enum as parameter"` test block in `checker.test.ts` pins the cascade: unannotated extern → `BuildError`, use-after-move → `"already been moved"`, symmetric branch consumption → no error.
