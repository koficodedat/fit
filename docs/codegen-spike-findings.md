...
- **Enum-linearity propagation** — closed (2026-05-27). Enum types whose variants carry linear payloads now resolve as `mode: "linear"`. Named bindings of such enums must be consumed before function return; the scope-exit enforcer catches the leak. Match scrutinees that are linear enum bindings are consumed by the match. See `tests/should_fail/enum_value_leak.fit` (rejected) and `tests/should_pass/enum_value_matched.fit` (accepted). One hole remains: an enum value that falls into an `if`/`match` branch and is not consumed on all branches may not be caught if the branch-merge logic doesn't see it as linear — track as a follow-on escalation trigger if it appears.
