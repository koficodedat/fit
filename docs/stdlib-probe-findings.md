...
`HttpConn` fits cleanly, and the checker accepts it with exit 0. The composite structure (HttpConn wrapping a TcpSocket and carrying HTTP-level state) works as designed. The probe used `sock: TcpSocket` (un-parameterized) as a workaround, attributed to a parser limitation. **That attribution was incorrect — `sock: TcpSocket<Connected>` was already parseable.** `parseType()` has supported `<typeArg>` in all type positions since the initial parser. The workaround was unnecessary; the parser was not the constraint. The actual open question — whether the outer resource's typestate should compositionally enforce anything about the inner field's typestate — remains deferred per the PoC charter. Inner-resource cleanup is trusted to the C stubs; the checker does not verify that HttpConn's cleanup transitively handles the socket.

#### server.fit: multi-type composition

server.fit successfully exercises all three resource types end-to-end in a single program, and the checker accepts it. The multi-type composition confirms that the typestate model, capability system, and lend/move inference compose correctly across module boundaries. ~~A previously undocumented limitation surfaced: **enum variant names must be globally unique**. `BrokenPipe` appeared in both `IoError` and `NetError`; `NotFound` appeared in both `IoError` and `HttpError`. These conflicts required renaming to `NetBrokenPipe` and `HttpNotFound`.~~ **[CLOSED: PoC 2026-05-29]** Variant name uniqueness was solved via §2.5 variant namespacing (Option B: dot syntax). server.fit now uses natural, un-prefixed names: `BrokenPipe` in both `IoError` and `NetError`, `NotFound` in both `IoError` and `HttpError`. The checker requires qualification only at ambiguous use sites; unambiguous bare names still resolve automatically. Multi-domain programs no longer degrade readability through forced prefixing. Deferred: whether mixed-qualification (bare and qualified names in the same match arm) is a design goal (see §2.5).
...
- ~~**Parameterized resource types as field types unsupported**~~ — **Misdiagnosed (2026-06-08).** `sock: TcpSocket<Connected>` is parseable in field position today and was at probe time. `parseType()` supports `<typeArg>` everywhere. The probe workaround was unnecessary. The real open question is composite typestate composition (deferred per PoC charter), not parser support.
...
| Limitation | Surfaced in | Status |
|-----------|------------|--------|
| Nested linear resources (stored-into-aggregate gap) | http.fit, server.fit | Workaround: used un-parameterized `TcpSocket` as HttpConn field; trusted C stubs for socket teardown. Open post-PoC design question. |
| **Enum variant name conflicts** (discovered probe, now closed) | server.fit | **CLOSED 2026-05-29:** Implemented §2.5 variant namespacing (Option B, dot syntax). server.fit reverted to natural names (`BrokenPipe`, `NotFound`); qualification required only at ambiguous sites. |
| Field access not supported | server.fit | Workaround: used `request_path(req)` free-function accessor. Open post-PoC design question. |
| ~~Parameterized resource types as field types unsupported~~ — **Misdiagnosed** | http.fit | `sock: TcpSocket<Connected>` is parseable today (was at probe time). Parser limitation was incorrectly attributed. Real open question: composite typestate composition (deferred). Regression guards: `field_parameterized_resource.fit`, `field_parameterized_in_resource.fit`. |
