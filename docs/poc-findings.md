...
The checker **enforces 10 semantic rules**. These are countable directly from the implementation and are language-independent — they describe what the language guarantees, not how the checker is written.
...
| 10 | **Records-cannot-own-linear** — a `record` may not declare a field of linear type: a resource, or an enum carrying a linear payload | `"record 'R' has linear field 'f' of type 'T' — records declare no cleanup and cannot own linear values; use a resource or an enum variant"` |
...
**FFI annotation cost at scale** — no-sigil lending is confirmed for FIT code; extern-annotation cost is bounded to the FFI boundary. **First measurement taken:** `examples/postgres/` — a ~200-line PostgreSQL v3.0 simple-query client over a minimal socket stdlib sketch — carries explicit annotations on 17 of 17 linear params on extern declarations, and 0 of 2 linear params on bodied declarations. That is what the pillar predicts: annotation cost concentrates at the FFI boundary and body-based inference covers bodied signatures. The sample is small (n=2 bodied params, one protocol), so this is a first data point rather than a closed question. Note that the example program is not evidence about readability — it was written by an author with the checker source in working memory, who reported steering around several known gaps; it also has not yet been through codegen.
...
| ~~**Records do not participate in linearity**~~ *(Closed in `aa5af91`. A record may not declare a field of linear type — resource, or enum with a linear payload. Enforced by a validation pass in `buildTypeEnv` placed after enum resolution, so linear-enum fields are caught alongside resources. Now rule 10. `FitType` still has no record variant and records still resolve to `plain/unrestricted`; the rule makes that safe by ensuring nothing linear can be inside one.)* | ~~Two independent unsoundnesses — leak and aliasing — both reachable with no new syntax.~~ Both are now rejected at the record declaration, before either behavior can manifest. The two motivating programs are retained as `tests/should_fail/record_linear_field_leak.fit` and `tests/should_fail/record_linear_field_alias.fit`; they now fail identically, at the declaration. | ~~Open design fork.~~ **Resolved: Option A.** Consistent with forced classification — types that own resources declare cleanup, records declare none, so records cannot own resources. Option B (records become linear, with recursive field cleanup) would have required field-level tracking, partial moves, and a defined cleanup order: three mechanisms to preserve one convenience. Cost in v0.1 is approximately nil, because there is no field-access syntax — `x.y` parses as `qualified_var` and no field-assignment form exists — so a resource inside a record was never retrievable from FIT code. Bundling a resource with data goes through a resource or an enum variant; general bundling semantics get designed alongside field-access syntax, against a concrete proposal. |
| **Aggregate field types discarded at codegen** — both the record and resource emission loops in `codegen.ts` hardcode `int` for every field regardless of declared type. `resource Outer { inner: Inner }` lowers to `typedef struct { int inner; } Outer;`. | Never surfaced before because every prior snapshot's fields were `Int`-ish, making `int` correct by accident. Applies to non-linear aggregate fields as well, so it survives either resolution of the records fork above. Verified by `examples/probes/record_field_codegen.fit` (`2622cb8`). | Emit `cTypeName(resolveType(f.type_, env))` per field. **Prerequisite: declaration ordering.** The record loop currently runs before the resource loop, so a record is emitted ahead of a resource it references — visible in probe 1.4's output, where `Wrapper` precedes the `Inner` it holds. Source order within each loop is also unchecked. Emitting correct field types without topological ordering (or forward declarations) produces C referencing undeclared types. The `int` erasure is currently masking this. |
