...
| **Records do not participate in linearity** — `record` decls are duplicate-checked in `buildTypeEnv` and otherwise ignored. They never enter `resources`, `aliases`, or `enumDecls`; `FitType` has no record variant; `resolveType` therefore falls through to `{ kind: "plain", mode: "unrestricted" }`. A record field of resource type is invisible to the checker. | Two independent unsoundnesses, both reachable with no new syntax — an extern returning the record is sufficient. **(1) Leak:** a linear value passed into a record is never required to be consumed. `examples/probes/record_leaks_linear.fit` is accepted with zero errors, and its generated C contains no cleanup call anywhere in the function body. **(2) Aliasing:** because the record is unrestricted it carries no use-once obligation, so the same wrapped resource can be handed out repeatedly — `examples/probes/record_aliases_linear.fit`, also accepted. An enum of identical shape is correctly rejected (`examples/probes/enum_holds_linear.fit`, one keyword apart from the leak probe). | **Open design fork.** Either forbid linear fields in records outright — consistent with forced classification, since types that own resources declare cleanup and records declare none — or make records linear with recursive field cleanup, which requires field-level tracking, partial moves, and a defined cleanup order. Verified `2622cb8`. |
